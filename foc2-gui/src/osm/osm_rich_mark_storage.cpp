...
namespace ImOsm::Rich {
    ...
    std::shared_ptr<MarkItem> MarkStorage::findMark(const std::string& name) const {
        const auto it = std::ranges::find_if(
            ...
            [name](const ItemNode& node) {
                return node.ptr->getText() == name;
            }
        );
        if (it != mark_items.end()) {
            return it->ptr;
        }
        return nullptr;
    }

    GeographicLib::GeoCoords MarkStorage::findMark(const std::string& name, bool& ok) const {
        if (const auto ptr = findMark(name)) {
            ok = true;
            return ptr->getCoords();
        }
        ok = false;
        return {};
    }
    ...
    void MarkStorage::addMark(const GeographicLib::GeoCoords& coords, const std::string& name) {
        mark_items.push_back({std::make_shared<MarkItem>(coords, name)});
    }
    ...
    bool MarkStorage::handleLoadState() {
        const bool loadState{load_state};
        load_state = false;
        return loadState;
    }

    bool MarkStorage::handlePickState() {
        const bool pickState{pick_state};
        pick_state = false;
        return pickState;
    }
}
