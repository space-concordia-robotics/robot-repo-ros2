...
from typing import Iterable, TypeVar, Generator, Callable, Sequence, Literal

from launch import LaunchDescription
from launch.actions import IncludeLaunchDescription, LogInfo, OpaqueFunction, DeclareLaunchArgument
...
def init_logging(enable: bool):
    ...
    logging.info(f"fmt: '{fmt}', style: '{style}', formatter: {formatter.__class__}")
...
class SimpleLauncher:
    ...
    @contextmanager
    def gz_bridge(self, name='gz_bridge') -> Generator[GazeboBridge, None, None]:
        """
            ...
            if len(im_topics):
                ...
                for topic in im_topics:
                    for ext in ('', '/compressed', '/compressedDepth', '/theora'):
                        remappings.append((SimpleSubstitution(topic.gz_topic, ext), SimpleSubstitution(topic.ros_topic, ext)))

                self.node(
                    f'{ros_gz}_image',
                    'image_bridge',
                    name=SimpleSubstitution(name, '_image'),
