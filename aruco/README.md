...
This package is used for detecting ArUco tags in ROS image topics.

It is largely derived from [aruco_opencv](https://github.com/fictionlab/ros_aruco_opencv), however it does have some notable changes:

- include rejected poses in the published message
- include corners in published message
- a bunch of misc. code cleanup
...
1. Prepare the camera topics.
   ...
   The ROS driver for the camera you are using should publish 2 topics containing: Camera images (most commonly`image_raw`, `image_color`, `image_mono` or
   `image_rect` topic) and Camera calibration data (`camera_info` topic). Both topics should be published in the same namespace. Let's assume the driver
   ...
   The camera should be calibrated (the better the calibration the better the marker pose estimation). If the camera is uncalibrated (the matrices D, K, R, P in
   `camera_info` messages are set to 0) or the calibration is invalid, you
   should [recalibrate the camera](https://navigation.ros.org/tutorials/docs/camera_calibration.html).
...
4. Visualize the data.
   ...
   The marker poses are also published on TF. You can visualize the data in RViz by setting fixed frame to the `frame_id` of the camera and adding the TF
   display.
