...
namespace autonomy {
        ...
        detection_subscription = node.create_subscription<ImageDetectionArray>(
            ...
            [this](const ImageDetectionArray::UniquePtr& msg) {
                const auto matched_tag = std::ranges::any_of(
                    msg->detections,
                    [&](const rover_msgs::msg::ImageDetection& marker) {
    ...
    rclcpp_async::Task<> ObjectTarget::start() {
        ...
        while (true) {
            ...
            if (const auto distance = util::distance(this->center, this->rover_position); distance <= this->object_config.acceptance_radius) {
                break;
            }
        }
    ...
    rclcpp_async::Task<std::optional<WrappedResult<NavigateThroughPoses>>> ObjectTarget::tryFindObject() const {
        ...
        goal_msg.poses = util::generateSpiral(
            ...
            this->radius,
            this->object_config.spiral_step,
            this->object_config.spiral_radius_factor
        );
    ...
    rclcpp_async::Task<std::optional<WrappedResult<NavigateThroughPoses>>> ObjectTarget::navigateToObject() {
        ...
        const auto detection = *std::ranges::find_if(
            aruco_detection->detections,
            [&](const rover_msgs::msg::ImageDetection& marker) {
                return marker.label == this->label;
            }
        );
        ...
        // TODO 2026-04-27 (Will Free): figure out how to navigate to it. depthai does not give us a pose for the object.

        // const auto tag_pose = detection.pose;
        ...
        // }
        //
        // rclcpp_action::Client<FollowGPSWaypoints>::SendGoalOptions options;
        // options.result_callback = [this](const rclcpp_action::Client<FollowGPSWaypoints>::WrappedResult& result) {
